...
export class AppManagerComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        this.dropFn = (e: any) => {
            ...
            if (e.dataTransfer.files.length) {
                ...
                reader.onload = (e: any) => {
                    // Render thumbnail.
                    try {
                        const json: BeatOnModJson = JSON.parse(e.target.result) as BeatOnModJson;
                        if (json.id) {
                            this.currentApp.packagename = 'com.beatonmod.' + json.id;
                        }
                        const searchTags: Materialize.ChipDataObject[] = [{ tag: 'BeatOn' }];
                        if (json.gameVersion) {
                            searchTags.push({
                        ...
                        this.service.showMessage({ error: true, data: 'Could not parse json!!' }, '');
                    }
                };
        ...
        this.sub = this.router.events.subscribe(async val => {
            if (val instanceof NavigationEnd) {
                ...
                if (this.apps_id) {
                    const apps = await this.expanseService.start().then(() => this.expanseService.getApp(this.apps_id));
                    if (!apps.length) {
                        this.apps_id = null;
                        ...
                        if (!this.currentApp.apk_url) {
                            this.currentApp.apk_url = await this.expanseService.getAppWebhook(this.apps_id);
                        }
            ...
            navigator.clipboard.writeText(this.currentApp.donate_url).then(
                () => {
                    this.service.showMessage(
                        { error: false },
                        isRefresh ? 'Share URL Refreshed and Copied to clipboard!' : 'Share URL Copied to clipboard!'
                    );
                },
                err => {
                    this.service.showMessage({ error: true, data: 'Cant copy share url!' }, '');
                }
            );
        }
    }
    ...
    copyUrl(url) {
        if (navigator.clipboard) {
            navigator.clipboard.writeText(url).then(
                () => {
                    this.service.showMessage({ error: false }, 'URL Copied to clipboard!');
                },
                err => {
                    this.service.showMessage({ error: true, data: 'Cant copy share url!' }, '');
                }
    ...
    findGitReleases() {
        ...
        this.isGettingGithub = true;
        return fetch(
            'https://api.github.com/repos/' + this.currentApp.github_name + '/' + this.currentApp.github_repo + '/releases'
        ).then(async r => {
            if (r.ok) {
        ...
        if (this.videoObject) {
            this.videoUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
                this.videoObject.provider === 'youtube'
                    ...
                    : 'https://player.vimeo.com/video/' + this.videoObject.id + '?byline=0&portrait=0'
            );
        }
    }
    ...
    backFillGithubRelease() {
        ...
        if (this.currentApp.github_enabled && this.hasGithubName && this.hasGithubRepo && releases.length) {
            let release = releases[0];
            if (this.currentApp.github_tag !== '[ all ]' && this.currentApp.github_tag !== '[ latest ]') {
            ...
            this.expanseService
                ...
                .then((res: any) => {
                    ...
                    if (!res.error && res.length) {
                        ...
                        this.refreshShareLink();
                        this.sendForApproval(res[0].apps_id).then(() => this.router.navigateByUrl('/my-app/' + res[0].apps_id));
                    }
                });
        }
    }
