...
import { AppCounter, AppUrl, ScreenShot, VideObject } from '../app-manager/app-manager.component';
...
export class EventListingComponent implements OnInit, OnDestroy {
    ...
    getMySubscription() {
        if (this.currentApp && this.events_id) {
            return this.expanseService.searchSubscribedEvents('', 0, this.events_id).then((res: any) => {
                if (res.length) {
    ...
    unsubscribe() {
        if (this.mySubscription) {
            this.expanseService
                ...
                .then(() => this.service.showMessage({}, 'Unsubscribed from this event!!'))
                .then(() => this.getMySubscription());
        }
    }
    ...
    attendingCount() {
        if (!this.event_meta.a) {
            return this.expanseService.eventCount('attending', this.events_id).then((res: any) => {
                if (!res.error) {
        ...
        if (!this.mySubscription) {
            this.expanseService.subscribeEvent(this.events_id).then(() => this.getMySubscription());
        }
    }

    viewEvent() {
        if (!this.event_meta.v) {
            return this.expanseService.eventCount('view', this.events_id).then((res: any) => {
                if (!res.error) {
    ...
    likeEvent() {
        if (!this.event_meta.l) {
            return this.expanseService.eventCount('like', this.events_id).then((res: any) => {
    ...
    async setupEvent() {
        if (this.events_id) {
            ...
            if (!events) {
                ...
                const counters = await this.expanseService.getEventTotals(this.events_id);
