...
export class AppListingComponent implements OnInit, OnDestroy {
    ...
    currentRating = 3;
    comforts: string[] = ['Comfortable', 'Moderate', 'Average', 'Exciting', 'Intense'];
    album: IAlbum[] = [];
    app_meta: any;
    stats = {
        clickthrough: 0,
    ...
    addReview() {
        ...
        this.expanseService
            ...
            .then((r: any) => {
                ...
                if (!r.error) {
                    return fetch(this.expanseService.discordURl + '/new_review/' + r.reviews_id);
                }
            })
            .then(() => {
                this.page = 0;
    ...
    copyShareUrl(isRefresh?) {
        if (navigator.clipboard) {
            navigator.clipboard.writeText(this.currentApp.donate_url).then(
                () => {
                    this.service.showMessage({ error: false }, 'Share URL Copied to clipboard!');
                },
                err => {
    ...
    downloadCount() {
        if (!this.app_meta.d) {
            return this.expanseService.appCount('download', this.apps_id).then((res: any) => {
                if (!res.error) {
                    this.service.app_meta[this.apps_id].d = 1;
                    this.service.app_meta[this.apps_id].vc = this.currentApp.versioncode;
                    this.counters.d++;
    ...
    likeApp() {
        if (!this.app_meta.l) {
            return this.expanseService.appCount('like', this.apps_id).then((res: any) => {
    ...
    subscribeToApp() {
        return this.service.isAuthenticated
            ? this.expanseService.addInstalledApp(this.apps_id, this.currentApp.versioncode).then(res => {
            ...
            : Promise.resolve().then(() => {
                  if (this.apk_download_urls && this.apk_download_urls.length) {
                      ...
                      this.service.showMessage(
                          { error: false },
                          this.service.hideLogo ? 'Installing...' : 'Sending to SideQuest Locally...'
                      );
                  }
              });
    }
    ...
    uninstallApp(packageName) {
        if (
            (this.currentApp.website === 'BeatOn' || this.currentApp.website === 'SynthRiders') &&
            this.currentApp.app_categories_id === '4'
        ) {
            this.expanseService.uninstallApp(this.apps_id).then(() => (this.isInstalled = false));
        } else {
            this.service
    ...
    async setupApp() {
        if (this.apps_id) {
            ...
            if (!apps.length) {
                ...
                this.donate_urls = this.app_urls.filter((url: AppUrl) => ['Patreon', 'Paypal', 'Kofi'].indexOf(url.provider) > -1);
