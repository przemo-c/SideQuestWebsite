...
export class SpaceListingComponent implements OnInit, OnDestroy {
    ...
    getMySubscription() {
        if (this.currentApp && this.spaces_id) {
            return this.expanseService.searchSubscribedSpaces('', 0, this.spaces_id).then((res: any) => {
                if (res.length) {
    ...
    attendingCount() {
        if (!this.space_meta.a) {
            return this.expanseService.eventCount('attending', this.spaces_id).then((res: any) => {
                if (!res.error) {
        ...
        if (!this.mySubscription) {
            this.expanseService.subscribeSpace(this.spaces_id).then(() => this.getMySubscription());
        }
    }

    viewSpace() {
        if (!this.space_meta.v) {
            return this.expanseService.spaceCount('view', this.spaces_id).then((res: any) => {
                if (!res.error) {
    ...
    likeSpace() {
        if (!this.space_meta.l) {
            return this.expanseService.spaceCount('like', this.spaces_id).then((res: any) => {
    ...
    async setupSpace() {
        if (this.spaces_id) {
            ...
            if (!events) {
                ...
                const counters = await this.expanseService.getSpaceTotals(this.spaces_id);

                if (this.currentApp.apps_id) {
                    this.expanseService.getApp(this.currentApp.apps_id).then(app => (this.selectedApp = app[0]));
                }
