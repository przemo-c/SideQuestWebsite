...
export class AppsToUpdateService {
  ...
  constructor(private expanseService: ExpanseClientService) {
    this.expanseService.installedAppsChangedAt.subscribe(
      (_timestamp: number) => {
        this.updateAppList();
      }
    );
  ...
  private async getAppsToUpdatePage(page: number): Promise<AppListing[]> {
    ...
    const isNotAMod = (app: AppListing) =>
      !(app.app_categories_id === "4" && app.website === "BeatOn");
    const allApps = (await this.expanseService.searchInstalledApps(
      "",
      page,
      true,
      false
