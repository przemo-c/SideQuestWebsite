...
export class AccountComponent implements OnInit, OnDestroy {
    ...
    setAvatarImage() {
        this.uploadService.uploadImage(true).then((res: any) => {
            ...
            if (existingParts.length > 1) {
                this.expanseService.savePreviewImage(res.fileId, this.expanseService.currentSession.avatar_images_id);
            }
        });
    }
    ...
    uploadIcon() {
        this.uploadService.uploadImage(true).then((res: any) => {
            this.expanseService.currentSession.banner_image = this.expanseService.cdnUrl + res.path;
            this.setBanner();
        });
    }
    ...
    removeUrl(key) {
        this.expanseService.removeUserValue('appUrl_' + key).then((res: any) => {
            ...
            if (!res.error && res.length) {
                this.expanseService.default_app_ulrs = this.expanseService.default_app_ulrs.filter(url => url.provider !== key);
            }
        });
    }
    ...
    getDefaultSpaces() {
        this.expanseService.start().then(() =>
            this.expanseService.getMySpaces(0, this.searchString).then(async (resp: SpaceListing[]) => {
    ...
    ngOnInit() {
        this.update_sub = this.appsToUpdateService.appsToUpdate.subscribe((apps: AppListing[]) => {
            this.appsToUpdateCount = apps.length;
        });
        this.expanseService
            .start()
            .then(() => {
                return this.expanseService.getSpace(this.expanseService.currentSession.default_space);
            })
            .then((space: SpaceListing) => (this.selectedSpace = space))
            .then(() => this.expanseService.getUserAppTotals())
            .then(t => {
                this.app_totals = t[0];
                Object.keys(this.app_totals).forEach(t => (this.app_totals[t] = Number(this.app_totals[t])));
            })
            .then(() => this.appService.getNotifications(this.expanseService));
    }

    setBanner() {
        this.expanseService.saveUserBannerImage(this.expanseService.currentSession.banner_image);
    }

    setPublic() {
        this.expanseService.saveUserPublicProfile(this.expanseService.currentSession.public_profile);
    }
    ...
    sendMessage() {
        this.expanseService.sendMessage(this.threadUsersId, this.messageToSend).then(resp => {
            this.page = 0;
            this.messageToSend = '';
    ...
    getRequests() {
        return this.getStuff('getFriendRequests').then(
            (friends: FriendListing[]) => (this.myRequests = this.page === 1 ? friends : this.myRequests.concat(friends))
        );
    }

    getBlocked() {
        return this.getStuff('getBlocked').then(
            (friends: FriendListing[]) => (this.myBlocked = this.page === 1 ? friends : this.myBlocked.concat(friends))
        );
    }

    getFriends() {
        return this.getStuff('getFriends').then(
            (friends: FriendListing[]) => (this.myFriends = this.page === 1 ? friends : this.myFriends.concat(friends))
        );
    }

    getMessages() {
        return this.getStuff('getMessagesPeople').then((messages: MessagePeopleListing[]) => {
            this.myMessages = this.page === 1 ? messages : this.myMessages.concat(messages);
        });
    }
    openMessageThread(users_id) {
        ...
        return this.expanseService
            ...
            .then(() => this.expanseService.getMessagesThread(users_id, this.page, this.searchString))
            .then((res: any) => {
                this.hasNoMore = !res.length;
                this.isLoading = false;
                if (res.length) {
                    res.forEach(r => {
                        r.is_mine = Number(r.users_id) === this.expanseService.currentSession.users_id;
                    });
                }
    ...
    getSubscribedSpaces() {
        return this.getStuff('searchSubscribedSpaces').then(
            (spaces: SpaceListing[]) =>
                (this.mySubscribedSpaces = this.page === 1 ? spaces : this.mySubscribedSpaces.concat(spaces))
        );
    }
    ...
    getSubscribedEvents() {
        return this.getStuff('searchSubscribedEvents').then(
            (events: EventListing[]) =>
                (this.mySubscribedEvents = this.page === 1 ? events : this.mySubscribedEvents.concat(events))
        );
    }

    setDefaultSpace() {
        this.expanseService.saveUserDefaultSpace(this.expanseService.currentSession.default_space);
    }
    ...
    getSpaces() {
        return this.getStuff('getMySpaces').then(
            (spaces: SpaceListing[]) => (this.mySpaces = this.page === 1 ? spaces : this.mySpaces.concat(spaces))
        );
    }
    ...
    getEvents() {
        this.isLoading = true;
        this.hasNoMore = false;
        return this.expanseService.getMyEvents(this.page, this.searchString, this.eventsType).then((res: any) => {
            this.hasNoMore = !res.length;
            this.isLoading = false;
    ...
    getInstalledApps() {
        ...
        return this.expanseService
            .start()
            .then(() => this.expanseService.searchInstalledApps(this.searchString, this.page, this.isUpdated, this.isUninstalled))
            .then((resp: AppListing[]) => {
                this.hasNoMore = !resp.length;
                this.isLoading = false;
                resp.forEach(a => {
                    a.image_url = a.image_url || this.expanseService.cdnUrl + 'file/2717/Untitled-2.jpg';
                });
                this.myInstalledApps = this.page === 0 ? resp : this.myInstalledApps.concat(resp);
    ...
    async setAppsToImport() {
        ...
        for (let i = 0; i < this.appsToImport.length; i++) {
            await this.expanseService.addInstalledApp(this.appsToImport[i].apps_id, this.appsToImport[i].versioncode);
    ...
    changePassword() {
        if (this.newPassword !== this.newPassword1 || this.newPassword.length < 6) {
            ...
            this.expanseService.saveUserPassword(this.newPassword).then(res => this.appService.showMessage(res, 'Password Saved!'));
        }
    }
    ...
    async checkForUpdates() {
        ...
        this.appsNeedingUpdated = this.myInstalledApps.filter(
            a => a.needsUpdate && !(a.app_categories_id === '4' && a.website === 'BeatOn')
        );
    }

    findGitReleases(app) {
        return fetch('https://api.github.com/repos/' + app.github_name + '/' + app.github_repo + '/releases').then(async r => {
            if (r.ok) {
                return r.json();
            }
        });
    ...
    public friendRequestCount() {
        return this.appService.notifications.friend_requests.length;
    }
}
