...
export class BannerCarouselComponent implements AfterViewInit {
    ...
    @ViewChild('slidePane', { static: false }) slidePane: ElementRef<HTMLDivElement>;
    @ViewChildren('slide') slides: QueryList<ElementRef<HTMLDivElement>>;
    @ViewChildren('slideImage') slideImages: QueryList<ElementRef<HTMLDivElement>>;
    ...
    private play(preloadOffset: SlideOffset) {
        ...
        this.slideTimeout = setTimeout(this.gotoSlideRelative.bind(this, 1), this.slideDelay);
    }

    private relativeSlideIndex(offset: SlideOffset) {
        return (this.activeIndex + this.slides.length + offset) % this.slides.length;
    }
}
